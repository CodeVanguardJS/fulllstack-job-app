...
export default function UpdateJobPage() {
  // --- Mock data, nanti ganti dari API fetch ---
  const mockJob = {
    jobTitle: "Frontend Developer",
    company: "PT Maju Jaya",
    location: "Jakarta / Remote",
    workType: "hybrid",
    description: "Responsible for building UI with React and Tailwind.",
  };
  ...
  // isi form dengan data awal (mock)
  useEffect(() => {
    setForm(mockJob);
  }, []);
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-5xl px-4 py-12">
        ...
        <form
          ...
          className="mx-auto max-w-2xl space-y-6 rounded-lg border border-slate-200 
                     bg-white/90 p-8 shadow-lg"
        >
          {/* Job Title */}
          <FormGroup
            id="jobTitle"
          ...
          <div>
            ...
            <select
              ...
              value={form.workType}
              onChange={handleChange}
              className="w-full rounded-md border border-gray-300 p-2 text-slate-800 
          ...
          <div className="flex justify-between pt-4">
            <button
              type="button"
              className="rounded-md border border-gray-300 px-4 py-2 text-slate-700 transition hover:bg-gray-100"
              onClick={() => setForm(mockJob)}
            >
              Reset
            </button>
            ...
            <button
              ...
              className="rounded-md bg-gradient-to-r from-indigo-600 to-violet-600 
                         px-4 py-2 font-semibold text-white transition 
...
function FormGroup({ id, label, placeholder, value, onChange }) {
  return (
    <div>
      <label className="mb-1 block font-semibold text-slate-700" htmlFor={id}>
        {label}
      </label>
      <input
        id={id}
